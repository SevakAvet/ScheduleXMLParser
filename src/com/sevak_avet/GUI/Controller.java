...
public class Controller implements Initializable {
	...
	private final EventHandler<ActionEvent> actionURL = new EventHandler<ActionEvent>() {
		@Override
	...
	private final EventHandler<ActionEvent> actionOpenFile = new EventHandler<ActionEvent>() {
		@Override
		public void handle(ActionEvent event) {				
			if(!periodsIsEmpty()) {
	...
	private final EventHandler<ActionEvent> actionSaveToFile = new EventHandler<ActionEvent>() {
		@Override
		public void handle(ActionEvent event) {
			FileChooser chooser = new FileChooser();
	...
	private final ChangeListener<String> choiceBoxTypeListener = new ChangeListener<String>() {
		@Override
	...
	private final ChangeListener<String> choiceBoxSessionListener = new ChangeListener<String>() {
		@Override
		public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		choiceBoxType.getSelectionModel().selectedItemProperty().addListener(choiceBoxTypeListener);
		
		choiceBoxSession.getItems().addAll("Летняя", "Зимняя");
		choiceBoxSession.getSelectionModel().select(0);
		chooseSession(summerSession);
		choiceBoxSession.getSelectionModel().selectedItemProperty().addListener(choiceBoxSessionListener);		
	}
	...
	private void setTFEnabled(boolean enabled) {
		if(enabled) {
			tfURL.setEditable(true);
			tfURL.setOpacity(1.0);
		} else {
			tfURL.setEditable(false);
			tfURL.setOpacity(0.5);
		}
	}
